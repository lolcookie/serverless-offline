...
class Offline {
    ...
    this.commands = {
      offline: {
        ...
        options: {
          ...
          cognitoIdentity: {
            usage:
              'Allows a custom cognitoIdentity, default is offlineContext_cognitoIdentityId',
          },
  ...
  _setOptions() {
    ...
    const defaultOpts = {
      ...
      noAuth: false,
      cognitoIdentity: 'offlineContext_cognitoIdentityId',
      corsAllowOrigin: '*',
  ...
  _createRoutes() {
    ...
    if (
      ['nodejs', 'nodejs4.3', 'nodejs6.10', 'nodejs8.10', 'babel'].indexOf(
        serviceRuntime
      ) === -1
    ) {
      this.printBlankLine();
        ...
        fun.events.forEach(event => {
          ...
          if (_.eq(event.http.private, true)) {
            protectedRoutes.push(
              `${event.http.method.toUpperCase()}#/${event.http.path}`
            );
          }
          ...
          this.server.route({
            ...
            handler: (request, reply) => {
              ...
              if (firstCall) {
                this.serverlessLog(
                  'The first request might take a few extra seconds'
                );
                firstCall = false;
              }
              ...
              // Check for APIKey
              if (
                _.includes(protectedRoutes, `${routeMethod}#${fullPath}`) ||
                _.includes(protectedRoutes, `ANY#${fullPath}`)
              ) {
              ...
              if (
                contentTypesThatRequirePayloadParsing.indexOf(contentType) !==
                -1
              ) {
                try {
              ...
              const lambdaContext = createLambdaContext(fun, (err, data) => {
                ...
                try {
                  ...
                  if (!this.options.dontPrintOutput) {
                    this.serverlessLog(
                      err
                        ? `Replying ${statusCode}`
              ...
              try {
                ...
                if (
                  (serviceRuntime === 'nodejs8.10' ||
                    serviceRuntime === 'babel') &&
                  !this.requests[requestId].done
                ) {
                  if (
                    x &&
  ...
  _configureAuthorization(endpoint, funName, method, epath, servicePath) {
    ...
    if (endpoint.authorizer) {
      ...
      if (
        typeof authFunctionName === 'string' &&
        authFunctionName.toUpperCase() === 'AWS_IAM'
      ) {
        this.serverlessLog(
      ...
      if (typeof endpoint.authorizer === 'object') {
        if (
          ...
          endpoint.authorizer.type.toUpperCase() === 'AWS_IAM'
        ) {
          this.serverlessLog(
            'WARNING: Serverless Offline does not support the AWS_IAM authorization type'
          );
        ...
        if (endpoint.authorizer.arn) {
          this.serverlessLog(
            `WARNING: Serverless Offline does not support non local authorizers: ${
              endpoint.authorizer.arn
            }`
          );
  ...
  _listen() {
    ...
    this.server.stop({ timeout: 5000 }).then(() => process.exit(this.exitCode));
  }
